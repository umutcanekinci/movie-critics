...
public class DatabaseManager {
    ...
    private void createTables() {
        try (InputStream in = getClass().getResourceAsStream("/schema.sql")) {
            ...
            // YENİ EKLENEN KISIM: Noktalı virgülleri sadece yeni bir CREATE veya INSERT başlıyorsa böl.
            String fileContent = new String(in.readAllBytes(), StandardCharsets.UTF_8);
            String[] statements = fileContent.split("(?i)(?<=;)\\s+(?=CREATE|INSERT)");
    ...
    private void execute(String sql, ThrowingPsSetter setter) {
        try (Connection con = connect(); PreparedStatement ps = con.prepareStatement(sql)) {
